...
def fetch_url(url: str, timeout: float) -> tuple[int | None, str, bytes]:
...
def probe_candidate(candidate: dict[str, Any], timeout: float) -> ProbeResult:
    ...
    try:
        http_status, content_type, body = fetch_url(url, timeout=timeout)
        ...
        elapsed_ms = int((time.monotonic() - start) * 1000)
        return ProbeResult(candidate_id, stream, source_class, url, "failed", None, "", "", 0, elapsed_ms, str(exc))
...
def main(argv: list[str] | None = None) -> int:
    ...
    results = [probe_candidate(candidate, timeout=args.timeout) for candidate in candidates]
