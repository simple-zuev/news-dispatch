...
def test_reader_model_validation_blocks_comment_feed_url() -> None:
    ...
    assert "comment feed URL" in str(report["issues"])


def test_reader_model_validation_blocks_generic_source_topic_title() -> None:
    item = safe_item()
...
def test_reader_model_validator_writes_report_and_returns_failure() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert report["passed"] is False
        assert "url is empty" in str(report["issues"])


def main() -> int:
    test_reader_model_validation_passes_safe_item()
    test_reader_model_validation_blocks_comment_feed_url()
    test_reader_model_validation_blocks_generic_source_topic_title()
